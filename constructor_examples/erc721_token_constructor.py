    ...
    def get_params(self):
        json_schema = {
            ...
            "properties": {
                ...
                #    "description": "Premint tokens will be sent to token owner. Leave blank for no premint. Tokens ids sent to the owner will start from 0."
                #},
                "is_burnable": {
                    "type": "boolean",
                    "default": False,
    ...
    def construct(self, fields):
        ...
        if fields['is_burnable'] and fields['is_pausable']:
            ...
            if fields['is_burnable']:
                parents.append('BurnableToken')

            if fields['is_pausable']:
                parents.append('PausableToken')

        constructor_inner_code = ''
        #if 'premint' in fields and fields['premint']:
        #    if 'max_tokens_count' in fields and fields['max_tokens_count'] and fields['max_tokens_count'] < fields['premint']:
        #        raise RuntimeError('premint tokens should be less then max tokens count')
        #
        #    constructor_inner_code = """
        #        uint256[] memory tokenIds = new uint[]({premint});
        #        {token_ids}
        #        _mintMultiple(msg.sender, tokenIds);
        #    """.format(
        #        premint=fields['premint'],
        #        token_ids='\n'.join(
        #            map(
        #                lambda i: '\t\ttokenIds[{i}] = {i};'.format(i=i),
        #                range(fields['premint'])
        #            )
        #        ),
        #    )
...
contract Token is ERC721Token %parents_code% {
    function Token()
        public
        payable
