...
class Constructor(ConstructorInstance):
    ...
    def get_params(self):
        json_schema = {
            "type": "object",
            "required": ["signs_count", "owners", "name", "symbol", "decimals"],
            ...
            "properties": {
                ...
                "has_approveAndCall": {
                    ...
                    "description": "This function allows token holder to pay other contract in tokens "
                                   "and immediately get desired actions from the called contract."
                },
                ...
                "frozen_premint": {
                    ...
                    "items": {
                        "type": "object",
                        "required": ["holder", "amount", "thaw_ts"],
                        "additionalProperties": False,
                        ...
                        "properties": {
                            ...
                            "thaw_ts": {
                                ...
                                "description": "On this time tokens of this holder will become available for "
                                               "transfer and spending.",
    ...
    def construct(self, fields):
        ...
        parent_contracts = 'ArgumentsChecker, multiowned'

        # multiowned

        source = safe_replace(self.__class__._TEMPLATE, '%signs_count%', str(fields['signs_count']))
        ...
        source = safe_replace(source, '%owners_code%', owners_code)

        # BurnableToken

        if fields.get('is_burnable'):
            parent_contracts += ', BurnableToken'
            source = safe_replace(source, '%BurnableToken_overridden%', '''
    /**
...
''')
        ...
        parent_contracts += ', StandardToken'

        source = safe_replace(source, '%name%', fields['name'])
        source = safe_replace(source, '%symbol%', fields['symbol'].upper())
        source = safe_replace(source, '%decimals%', str(fields['decimals']))
        source = safe_replace(source, '%premint2sender%', str(fields.get('premint2sender', 0)))

        # TokenWithApproveAndCallMethod

        if fields.get('has_approveAndCall'):
            parent_contracts += ', TokenWithApproveAndCallMethod'

        source = safe_replace(source, '%parent_contracts%', parent_contracts)

        # frozen_premint

        frozen_premint = []
        frozen_premint_total = 0
        for add_frozen in fields.get('frozen_premint', []):
            frozen_premint_total += int(add_frozen['amount'])
            frozen_premint.append('addFrozen(address({}), {}, {}, false);'.format(
                add_frozen['holder'], add_frozen['amount'], add_frozen['thaw_ts']))

        frozen_premint = '\n'.join(frozen_premint)
        frozen_premint += '\ntotalSupply = totalSupply.add({});'.format(frozen_premint_total)
        source = safe_replace(source, '%frozen_premint%', frozen_premint)
...
contract SmartzToken is %parent_contracts% {
